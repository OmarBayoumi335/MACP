...
def initEnigmaServer():
    ...
    for key, value in request.args.items():
        args[key] = value
    enigmaServer = EnigmaServer(args)
    if args["req"][:3] == "get":
        return enigmaServer.get()
    if args["req"][:3] == "pos":
        return enigmaServer.post()
    if args["req"][:3] == "put":
        return enigmaServer.put()
    if args["req"][:3] == "del":
        return enigmaServer.delete()
    return "failure"
